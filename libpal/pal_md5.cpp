...
static void MD5Transform(uint32_t state[4], unsigned char buffer[64]);
static void Encode(unsigned char*, const uint32_t*, unsigned int);
static void Decode(uint32_t*, const unsigned char*, unsigned int);
static void MD5_memcpy(unsigned char* dst, unsigned char* src, unsigned int bytes);
static void MD5_memset(unsigned char* dst, int value, unsigned int bytes);
...
void palMD5::Update(unsigned char* input, unsigned int inputLen) {
  unsigned int i;
  unsigned int index;
  ...
  count[0] += (uint32_t)inputLen << 3;
  if (count[0] < ((uint32_t)inputLen << 3)) {
    count[1]++;
  }
...
void palMD5::GetMD5(unsigned char* digest) {
